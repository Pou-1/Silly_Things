...
public class GiantKiwiAI : EnemyAI, IVisibleThreat
{
	...
	public override void Start()
	{
		...
		if (StartOfRound.Instance.inShipPhase)
		{
			...
			if (birdNest == null)
			{
				birdNest = UnityEngine.Object.FindObjectOfType<EnemyAINestSpawnObject>().gameObject;
				agent.enabled = false;
...
{if(playerWhoSent != (int)GameNetworkManager.Instance.localPlayerController.playerClientId)			{
	...
	private void CalculateLookingAnimation()
	{
		...
		bool flag2 = currentBehaviourStateIndex == 2 && Vector3.Distance(base.transform.position, destination) > 4f && Mathf.Abs(velX) > 0.3f;
		...
		{
			if (watchingThreat == null || flag || @bool || Physics.Linecast(eye.position, watchingThreat.GetThreatLookTransform().position, lookMask, QueryTriggerInteraction.Ignore))
			{
				agent.angularSpeed = 520f;
				headLookRig.weight = Mathf.Lerp(headLookRig.weight, 0f, Time.deltaTime * 16f);
		...
		if (base.IsOwner && !flag2 && Vector3.Angle(base.transform.forward, Vector3.Scale(new Vector3(1f, 0f, 1f), lookTarget.position - base.transform.position)) > 4f * Mathf.Min(Vector3.Distance(base.transform.position, lookTarget.position) * 0.5f, 10f))
		{
